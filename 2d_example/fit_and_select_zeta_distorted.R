...
Ynoisy = raw_data$Ynoisy
Cnoisy = raw_data$Cnoisy
J = length(unique(Cnoisy))

# Range of tuning parameter zeta:
zeta_seq = c(0.2, 1)
# Fit model for each zeta, compute SSE and KL-div:
if (do_estimate) {
...
#    pmc = list(npart = 5, nburn = 3500, nsave = ns, nskip = 1, ndisplay = 100)
    ...
    try({
      # Fit model:
      res = COMIX::comix(Ynoisy, Cnoisy, prior = prior, pmc = pmc)
      ...
      saveRDS(rr, paste0("./fitted_distorted/cal_data_zeta_", zeta, ".rds"))
    })
  }
} else {
  lz = length(zeta_seq)
  ...
  for (zeta in zeta_seq) {
    try({
      rr = readRDS(paste0("./fitted_distorted/cal_data_zeta_", zeta, ".rds"))
      K_num[idx,] = rr$K_num
    })
    idx = idx + 1
  }
...
# Figure 5:
pdf("Figure_5.pdf", width = 6, height = 4)
demo_idx = c(1,2)
if (plot_compare) {
  winner_idx = demo_idx
  ...
  for (idx in winner_idx) {
    rr = readRDS(paste0("./fitted_distorted/cal_data_zeta_",
                        zeta_seq[idx],
                        ".rds"))
    Y_cal = rr$Ycal
    ust = unique(rr$sum_chain$t)
